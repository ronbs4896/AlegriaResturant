...
-- ── תשלומים (מיגרציה 0003) ─────────────────────────────────
-- ALTER ולא עמודה בתוך ה-CREATE, כדי שמסד פיתוח קיים יקבל את
-- העמודות החדשות לפני האינדקס שנשען עליהן.
...
CREATE INDEX IF NOT EXISTS documents_payment_idx ON documents (payment_status, due_date);
...
CREATE INDEX IF NOT EXISTS document_payments_document_idx ON document_payments (document_id, paid_at);
CREATE INDEX IF NOT EXISTS document_payments_paid_at_idx ON document_payments (paid_at);

UPDATE documents SET payment_status = 'paid', paid_at = COALESCE(paid_at, doc_date)
  WHERE doc_kind IN ('receipt', 'tax_invoice_receipt') AND payment_status <> 'paid';
UPDATE documents SET payment_status = 'n/a'
  WHERE doc_kind = 'credit_note' AND payment_status <> 'n/a';
